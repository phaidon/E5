...
{if count($smilies) > 0}
<div id="smiley_bar" class="e5_bar2" style="display: none;">
<div style=float:left;" id='smiley_spacer'>
    {img modname='E5' src="spacer.png" title='spacer'}
</div>
{foreach from=$smilies item="icon" key="tag"}
    {img modname='E5' src="smilies/$icon" title=$tag 
    onclick="document.execCommand('inserthtml', false, '$tag');return false" class='smiley_class'}
{/foreach}
</div>
...
{foreach from=$elements item="e" key="tag"}
<div id="{$tag}_bar" class="e5_bar2"" style="display:none;">
    <div style=float:left;" id='{$tag}_spacer'>
        {img modname='E5' src="spacer.png" title='spacer'}
    </div>
    {if $e.type == 'bar'}
        {include file="tags/$tag.tpl"}
